...
nel gruppo con id *${msg.chat.id}* verrà mandato un recap giornaliero in caso di allerte locali nella zona *${geoloc}*!`
      ...
      try {
        await botService.sendHTMLMessage(msg.chat.id, 'Il geocode non è valido.\n\n<code>/set [GEOCODE]</code> per impostare una zona.\n<b>Ex. </b><code>/set Lomb-09</code>')
      } catch (e) {
...
bot.command('help', async (ctx) => {
  ...
  try {
    await botService.sendHTMLMessage(msg.chat.id,
...
bot.command('update', async (ctx) => {
  ...
  try {
    ...
    logger.error(`Force update request throw an error on group ${msg.chat.id}`, err);
  }
})

bot.command('unsync', async (ctx) => {
  const msg = ctx.message
  logger.info(`Unsync command requested by ${msg.from.id} on group ${msg.chat.id}`);
  const chat = await chatService.findChat(msg.chat.id)
  if(chat.data) {
    try {
      await chatService.deleteChat(ctx)
      await botService.sendHTMLMessage(msg.chat.id, 'Richiesta di cancellazione dalle allerte automatiche ricevuta.\n\nUsa <code>/set [GEOCODE]</code> per riattivare il servizio su una zona.\n<b>Ex. </b><code>/set Lomb-09</code>\n\nPuoi anche richiedere allerte su richiesta tramite il comando <code>/info</code>')
    } catch (err) {
      logger.error(`Unsync request throw an error on group ${msg.chat.id}`, err);
    }
  } else {
    await botService.sendHTMLMessage(msg.chat.id, 'Questa chat non è ancora stata configurata e non può quindi essere scollegata dalle allerte periodiche.\n\nUsa il comando <code>/set [GEOCODE]</code> per impostare una zona.\n<b>Ex. </b><code>/set Lomb-09</code>')
    logger.error(`Unsync request throw an error for unset chat on group ${msg.chat.id}`);
  }
})
